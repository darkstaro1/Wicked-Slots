...
class Machine:
    ...
    def input(self):
        ...
        # Checks for mouse click anywhere on the screen
        if mouse_click[0] and self.can_toggle and self.currPlayer.balance >= self.currPlayer.bet_size:
            self.toggle_spinning()
            self.spin_time = pygame.time.get_ticks()
            self.currPlayer.place_bet()
            self.machine_balance += self.currPlayer.bet_size
    ...
    def check_wins(self, result):
        ...
        for row in horizontal:
            for sym in row:
                if row.count(sym) > 3:  # Potential win
